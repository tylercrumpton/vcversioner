...
def test_astounding_success(gitdir):
    ...
    version = tcversioner.find_version(Popen=basic_version)
...
def test_no_git(gitdir):
    ...
    with pytest.raises(SystemExit) as excinfo:
        tcversioner.find_version(Popen=empty)
...
def test_when_Popen_raises(gitdir):
    ...
    with pytest.raises(SystemExit) as excinfo:
        tcversioner.find_version(Popen=RaisingFakePopen())
    assert excinfo.value.args[0] == 2
    assert not gitdir.join('version.txt').check()
...
def test_no_git_but_version_file(gitdir):
    ...
    version = tcversioner.find_version(Popen=empty)
    assert version == ('1.0', '0', 'gbeef')
...
def test_Popen_raises_but_version_file(gitdir):
    ...
    with gitdir.join('version.txt').open('w') as outfile:
        outfile.write('1.0-0-gbeef')
    version = tcversioner.find_version(Popen=RaisingFakePopen())
    assert version == ('1.0', '0', 'gbeef')
...
def test_version_file_with_root(gitdir):
    ...
    with gitdir.join('version.txt').open('w') as outfile:
        outfile.write('1.0-0-gbeef')
    version = tcversioner.find_version(
...
def test_invalid_git(gitdir):
    ...
    with pytest.raises(SystemExit) as excinfo:
        tcversioner.find_version(Popen=invalid)
    assert excinfo.value.args[0] == 2
    assert not gitdir.join('version.txt').check()
...
def test_dev_version(gitdir):
    ...
    version = tcversioner.find_version(Popen=dev_version)
...
def test_dev_version_disabled(gitdir):
    ...
    version = tcversioner.find_version(Popen=dev_version, include_dev_version=False)
...
def test_custom_vcs_args(gitdir):
    ...
    with pytest.raises(SystemExit):
        tcversioner.find_version(Popen=popen, vcs_args=('foo', 'bar'))
...
def test_custom_vcs_args_substitutions(gitdir):
    ...
    popen = RaisingFakePopen()
    with pytest.raises(SystemExit):
        tcversioner.find_version(Popen=popen, vcs_args=('foo', 'bar', '%(pwd)s', '%(root)s'))
...
def test_custom_vcs_args_substitutions_with_different_root(tmpdir):
    ...
    popen = RaisingFakePopen()
    with pytest.raises(SystemExit):
        tcversioner.find_version(Popen=popen, root='/spam', vcs_args=('%(root)s',))
    assert popen.args[0] == ['/spam']

def test_custom_version_file(gitdir):
    "The version.txt file can have a unique name."
    version = tcversioner.find_version(Popen=basic_version, version_file='custom.txt')
    assert version == ('1.0', '0', 'gbeef')
...
def test_custom_version_file_reading(gitdir):
    ...
    with gitdir.join('custom.txt').open('w') as outfile:
        outfile.write('1.0-0-gbeef')
    version = tcversioner.find_version(Popen=empty, version_file='custom.txt')
    assert version == ('1.0', '0', 'gbeef')

def test_version_file_disabled(gitdir):
    "The version.txt file can be disabled too."
    version = tcversioner.find_version(Popen=basic_version, version_file=None)
    assert version == ('1.0', '0', 'gbeef')
    assert not gitdir.join('version.txt').check()
...
def test_version_file_disabled_git_failed(gitdir):
    ...
    with pytest.raises(SystemExit) as excinfo:
        tcversioner.find_version(Popen=empty, version_file=None)
    assert excinfo.value.args[0] == 2
    assert not gitdir.join('version.txt').check()
...
def test_version_file_disabled_Popen_raises(gitdir):
    ...
    with pytest.raises(SystemExit) as excinfo:
        tcversioner.find_version(Popen=RaisingFakePopen(), version_file=None)
    assert excinfo.value.args[0] == 2
    assert not gitdir.join('version.txt').check()
...
def test_namedtuple(tmpdir):
    ...
    tmpdir.chdir()
    version = tcversioner.find_version(Popen=basic_version, version_file=None, vcs_args=[])
...
def test_namedtuple_nonzero_commits(tmpdir):
    ...
    tmpdir.chdir()
    version = tcversioner.find_version(Popen=dev_version, version_file=None, vcs_args=[])
...
def test_git_arg_path_translation(gitdir, monkeypatch):
    ...
    popen = RaisingFakePopen()
    with pytest.raises(SystemExit):
        tcversioner.find_version(Popen=popen, vcs_args=['spam/eggs'], version_file=None)
...
def test_version_file_path_translation(gitdir, monkeypatch):
    ...
    with pytest.raises(OSError):
        tcversioner.find_version(Popen=basic_version, open=open, version_file='spam/eggs', vcs_args=[])
...
def test_git_output_on_no_version_file(gitdir, capsys):
    ...
    with pytest.raises(SystemExit):
        tcversioner.find_version(Popen=git_failed, version_file=None, vcs_args=[])
...
def test_git_output_on_version_file_absent(gitdir, capsys):
    ...
    with pytest.raises(SystemExit):
        tcversioner.find_version(Popen=git_failed, version_file='version.txt', vcs_args=[])
    out, err = capsys.readouterr()
    assert not err
    assert out == (
...
def test_no_git_output_on_version_unparsable(capsys):
    ...
    with pytest.raises(SystemExit):
        tcversioner.find_version(Popen=invalid, version_file='version.txt', vcs_args=[])
    out, err = capsys.readouterr()
    assert not err
    assert out == (
...
def test_no_output_on_success(gitdir, capsys):
    ...
    tcversioner.find_version(Popen=basic_version)
    out, err = capsys.readouterr()
    assert not out
    assert not err
...
def test_no_output_on_version_file_success(gitdir, capsys):
    ...
    tcversioner.find_version(Popen=git_failed)
    out, err = capsys.readouterr()
    assert not out
    assert not err
...
def test_strip_leading_v(gitdir):
    ...
    version = tcversioner.find_version(Popen=FakePopen(b'v1.0-0-gbeef'))
    assert version == ('1.0', '0', 'gbeef')

def test_strip_leading_prefix(gitdir):
    "The leading prefix stripped from tags can be customized."
    version = tcversioner.find_version(Popen=FakePopen(b'debian/1.0-0-gbeef'), strip_prefix='debian/')
    assert version == ('1.0', '0', 'gbeef')

def test_git_args_deprecation(gitdir):
    "git_args is deprecated."
    pytest.deprecated_call(tcversioner.find_version, git_args=['git', 'spam'], Popen=basic_version)

def test_git_args_still_works(gitdir):
    "git_args still works like vcs_args."
    popen = RaisingFakePopen()
    with pytest.raises(SystemExit):
        tcversioner.find_version(git_args=['git', 'spam'], Popen=popen)
...
def test_hg_detection(hgdir):
    ...
    popen = RaisingFakePopen()
    with pytest.raises(SystemExit):
        tcversioner.find_version(Popen=popen)
    assert popen.args[0] == [
...
def test_no_vcs_no_version_file(tmpdir, capsys):
    ...
    tmpdir.chdir()
    with pytest.raises(SystemExit):
        tcversioner.find_version(version_file=None, Popen=basic_version)
    out, err = capsys.readouterr()
    assert not err
    assert out == (
...
def test_no_vcs_absent_version_file(tmpdir, capsys):
    ...
    tmpdir.chdir()
    with pytest.raises(SystemExit):
        tcversioner.find_version(version_file='version.txt', Popen=basic_version)
    out, err = capsys.readouterr()
    assert not err
    assert out == (
...
def test_decrement_dev_version(gitdir):
    ...
    version = tcversioner.find_version(decrement_dev_version=True, Popen=dev_version)
...
def test_decrement_dev_version_to_zero(gitdir):
    ...
    version = tcversioner.find_version(decrement_dev_version=True, Popen=FakePopen(b'1.0-1-gbeef'))
    assert version == ('1.0', '0', 'gbeef')
...
def test_automatic_decrement_dev_version_with_hg(hgdir):
    ...
    version = tcversioner.find_version(Popen=hg_version)
...
def test_automatic_decrement_dev_version_disabled(hgdir):
    ...
    version = tcversioner.find_version(decrement_dev_version=False, Popen=hg_version)
...
def test_dev_suffix_with_patch(gitdir):
    ...
    version = tcversioner.find_version(decrement_dev_version=False, use_dev_not_post=True, Popen=FakePopen(b'1.0.0-1-gbeef'))
...
def test_dev_suffix_with_alpha(gitdir):
    ...
    version = tcversioner.find_version(decrement_dev_version=False, use_dev_not_post=True, Popen=FakePopen(b'1.0.0a1-1-gbeef'))
...
def test_dev_suffix_with_long_patch(gitdir):
    ...
    version = tcversioner.find_version(decrement_dev_version=False, use_dev_not_post=True,
